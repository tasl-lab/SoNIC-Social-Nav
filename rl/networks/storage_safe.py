...
class RolloutStorage(object):
    ...
    def __init__(self, num_steps, num_processes, obs_shape, action_space,
        ...
        self.costs = torch.zeros(num_steps, num_processes, 1)
        self.value_preds = torch.zeros(num_steps + 1, num_processes, 1)
        self.cost_value_preds = torch.zeros(num_steps + 1, num_processes, 1)
        self.returns = torch.zeros(num_steps + 1, num_processes, 1)
        self.cost_returns = torch.zeros(num_steps + 1, num_processes, 1)
    ...
    def insert(self, obs, recurrent_hidden_states, actions, action_log_probs,
               value_preds, cost_value_preds, rewards, costs, masks, bad_masks):
    ...
    def compute_returns(self,
        ...
        if use_proper_time_limits:
            ...
            if use_gae:
                self.value_preds[-1] = next_value
                self.cost_value_preds[-1] = cost_next_value
                gae = 0
                cost_gae = 0
                for step in reversed(range(self.rewards.size(0))):
                    delta = self.rewards[step] + gamma * self.value_preds[
                    ...
                    cost_delta = self.costs[step] + gamma * self.cost_value_preds[
                        step + 1] * self.masks[step +
                                               1] - self.cost_value_preds[step]
                    cost_gae = cost_delta + gamma * gae_lambda * self.masks[step +
                                                                  1] * cost_gae
                    self.cost_returns[step] = cost_gae + self.cost_value_preds[step]
            else:
                # self.returns[-1] = next_value
                # for step in reversed(range(self.rewards.size(0))):
                #     self.returns[step] = self.returns[step + 1] * \
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            ...
            cost_value_preds_batch = self.cost_value_preds[:-1].view(-1, 1)[indices]
            return_batch = self.returns[:-1].view(-1, 1)[indices]
            cost_return_batch = self.cost_returns[:-1].view(-1, 1)[indices]
            ...
            if cost_advantages is None:
                cost_adv_target = None
            else:
                cost_adv_target = cost_advantages.view(-1, 1)[indices]

            yield obs_batch, recurrent_hidden_states_batch, actions_batch, \
                value_preds_batch, cost_value_preds_batch, return_batch, cost_return_batch, \
                    masks_batch, old_action_log_probs_batch, adv_targ, cost_adv_target
    ...
    def recurrent_generator(self, advantages, cost_advantages, num_mini_batch):
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            for offset in range(num_envs_per_batch):
                ...
                cost_value_preds_batch.append(self.cost_value_preds[:-1, ind])
                ...
                cost_adv_targ.append(cost_advantages[:, ind])
            ...
            cost_value_preds_batch = torch.stack(cost_value_preds_batch, 1)
            return_batch = torch.stack(return_batch, 1)
            cost_return_batch = torch.stack(cost_return_batch, 1)
            masks_batch = torch.stack(masks_batch, 1)
            old_action_log_probs_batch = torch.stack(
                old_action_log_probs_batch, 1)
            adv_targ = torch.stack(adv_targ, 1)
            cost_adv_targ = torch.stack(cost_adv_targ, 1)
            ...
            cost_value_preds_batch = _flatten_helper(T, N, cost_value_preds_batch)
            return_batch = _flatten_helper(T, N, return_batch)
            cost_return_batch = _flatten_helper(T, N, cost_return_batch)
            masks_batch = _flatten_helper(T, N, masks_batch)
            old_action_log_probs_batch = _flatten_helper(T, N, \
                    old_action_log_probs_batch)
            adv_targ = _flatten_helper(T, N, adv_targ)
            cost_adv_targ = _flatten_helper(T, N, cost_adv_targ)

            yield obs_batch, recurrent_hidden_states_batch, actions_batch, \
                value_preds_batch, cost_value_preds_batch, return_batch, cost_return_batch, \
                    masks_batch, old_action_log_probs_batch, adv_targ, cost_adv_targ
